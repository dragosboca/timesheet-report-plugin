...
export class TimesheetReportView extends ItemView {
    ...
    async renderReport() {
        ...
        // Remove previous refresh button if it exists
        const oldRefreshButton = this.contentEl.querySelector('.timesheet-refresh-button');
        if (oldRefreshButton) {
            oldRefreshButton.remove();
        }

        // Remove previous button container if it exists
        const oldButtonContainer = this.contentEl.querySelector('.timesheet-button-container');
